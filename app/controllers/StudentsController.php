...
public function search()
{
    ...
    if (!empty($results)) {
        foreach ($results as $row) {
            ...
            echo '<td style="text-align:center; color: #000;;">' . htmlspecialchars($fullName) . '</td>';

            // Email
            echo '<td style="text-align:center; color: #000;;">' . htmlspecialchars($row['emails']) . '</td>';

            // Action buttons
            echo '<td style="text-align:center;">';
            echo '<a href="' . site_url('/update/'.$row['id']) . '" 
                     class="btn update" style="background:#6c63ff; padding:6px 14px; border-radius:8px; color: #000;; text-decoration:none; margin:2px;"> Update</a>';
            echo '<a href="' . site_url('/delete/'.$row['id']) . '" 
                     class="btn delete" 
                     style="background:#ff6b6b; padding:6px 14px; border-radius:8px; color: #000;; text-decoration:none; margin:2px;"
